    ...
    if (annotateTop == FALSE) {
      if (logp) {
        with(subset(d, P <= annotatePval), 
             textxy(pos, -log10(P), offset = 0.625, labs = topHits$SNP, cex = 0.9), ...)
      } else
        with(subset(d, P >= annotatePval), 
             textxy(pos, P, offset = 0.625, labs = topHits$SNP, cex = 0.9), ...)
    }
    else {
      ...
      if (logp ){
        textxy(topSNPs$pos, -log10(topSNPs$P), offset = 0.625, labs = topSNPs$SNP, cex = 0.9, ...)
      } else
        textxy(topSNPs$pos, topSNPs$P, offset = 0.625, labs = topSNPs$SNP, cex = 0.9, ...)
    }
  }  
  par(xpd = FALSE)
}